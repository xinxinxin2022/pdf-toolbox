...
export default function FAQ({ items }: FAQProps) {
  ...
  return (
    <div className="space-y-2">
      {items.map((item, i) => {
        // Support both i18n-key based and pre-resolved items
        const question = item.q || (item.qKey ? t(item.qKey) : '');
        const answer = item.a || (item.aKey ? t(item.aKey) : '');
        return (
          <div key={i} className="border border-neutral-100 dark:border-neutral-800 rounded-xl overflow-hidden">
            ...
            >
              <span className="text-[15px] font-medium text-neutral-900 dark:text-white pr-4">{question}</span>
              <ChevronDown
                size={18}
            ...
            {openIndex === i && (
              <div className="px-5 pb-4 bg-white dark:bg-neutral-900">
                <p className="text-[14px] text-neutral-500 dark:text-neutral-400 leading-relaxed">{answer}</p>
              </div>
            )}
          </div>
        );
